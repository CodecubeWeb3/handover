...
Implementation: booking transitions invoke the shared payment manager to ensure two PaymentIntent records (A/B) at open_A_window and capture them at scan_B_ok, updating payments, payment_intents, and a pending transfers row. Stripe interaction is stubbed with deterministic ids until real gateway credentials are supplied. Late-fee charges are derived from recorded scan event timestamps against country grace/wait caps and persisted alongside the capture.
...
Cancel B PI. Implementation: booking Timer A grace expiry calls SharedPaymentManager::captureForNoShowA, capturing the higher of the min-capture threshold or late_fee_A+stipend and cancelling leg B.

NO_SHOW_B:

Capture B: slot_price + late_fee_B + stipend. Implementation: Timer B grace expiry drives SharedPaymentManager::captureForNoShowB to settle leg B while leaving leg A as previously captured.
...
Private thread per booking. No phone/email until confirmed. `thread.message`, `thread.typing`, `thread.read`, and `thread.flagged` events broadcast on `private-booking.{id}` channels; web UI available at `/messages` (authenticated).
...
POST /api/bookings/{booking}/scan/{leg}
- Auth: operative/admin/moderator via Sanctum token.
- Body: event_uuid (UUID), token.code (TOTP or offline PIN), location.lat/lng (required), device.attested (bool), optional metadata hash.
- Guard outcomes merged into context; event_uuid enforces idempotency (replays return prior event payload).

GET /api/bookings/{booking}/pass/{leg}
- Auth: request parent, assigned operative, admin/moderator.
- Returns otpauth URI, QR payload, secret, offline PIN, deep link, rotation/expiry timestamps.
- Query ?refresh=1 forces rotation + queues notifications.
...
POST /api/messages/{thread}/typing

POST /api/messages/{thread}/read

POST /api/messages/flag/{message}
...
RotateStalePassTokensJob (refresh pass secrets + notify parties).
...
Run `php artisan app:install` to launch the guided setup. The command validates PHP extensions, collects environment details, writes `.env`, runs key:generate, migrations, seeds, and storage:link, then prints cron/queue/webhook instructions.
Check PHP 8.4 + extensions: pdo_mysql, openssl, mbstring, tokenizer, xml,
ctype, json, gd, curl, bcmath.

Collect DB, APP_URL, mail, queue, Stripe, Pusher/Ably, SMS, S3 keys, pass token issuer/rotation env (PASS_TOKEN_ISSUER, PASS_TOKEN_ROTATION_SECONDS, PASS_TOKEN_TOTP_WINDOW, PASS_TOKEN_DEEPLINK_SCHEME).
...
Health check endpoint: GET /api/health returns JSON status for app, database, cache, and queue.
...
Stripe: preauth, capture, transfer, refund, idempotent webhooks. Refunds currently routed through PaymentRefundService pending live Stripe integration. Enable full Stripe mode by setting `PAYMENT_GATEWAY=stripe` and configuring webhook secrets/keys; default `simulation` mode keeps API calls local.
